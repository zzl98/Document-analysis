...
class DepParser(object):
    ...
    def left_arc(self):
        ...
        if not self.ctx.is_buffer_empty():
            if self.ctx.stack_top() == 0:
                return False
            if self.ctx.has_dependency_with_target(self.ctx.stack_top()):
                return False
            if not (self.ctx.word_id_to_str(self.ctx.next_buffer_word_id()),self.ctx.word_id_to_str(self.ctx.stack_top())) in self.rules:
                return False
            self.ctx.add_dependency(self.ctx.next_buffer_word_id(), self.ctx.stack_top())
            self.ctx.pop_stack()
            return True
        return False
    ...
    def right_arc(self):
        ...
        if not self.ctx.is_buffer_empty():
            if not (self.ctx.word_id_to_str(self.ctx.stack_top()),self.ctx.word_id_to_str(self.ctx.next_buffer_word_id())) in self.rules:
                return False
            if self.ctx.has_dependency_with_target(self.ctx.next_buffer_word_id()):
                return False
            self.ctx.add_dependency(self.ctx.stack_top(), self.ctx.next_buffer_word_id())
            self.ctx.push_stack(self.ctx.next_buffer_word_id())
            self.ctx.pop_buffer()
            return True
        return False
    ...
    def reduce(self):
        ...
        if not self.ctx.is_buffer_empty():
            if not self.ctx.has_dependency_with_target(self.ctx.stack_top()):
                return False
            self.ctx.pop_stack()
            return True
        return False
